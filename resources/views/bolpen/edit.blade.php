...
@section('konten')
    ...
    <form action="{{ route('bolpen.update', $bolpen->KodeBolpen) }}" method="POST" onsubmit="return validasiForm()">
        @csrf
        @method('PUT')

        <p>
            <label>Merk Bolpen</label><br>
            <input type="text" name="MerkBolpen" id="MerkBolpen" maxlength="30" value="{{ old('MerkBolpen', $bolpen->MerkBolpen) }}">
        </p>

        <p>
            <label>Stock Bolpen</label><br>
            <input type="number" name="StockBolpen" id="StockBolpen" value="{{ old('StockBolpen', $bolpen->StockBolpen) }}">
        </p>

        <p>
            <label>Tersedia</label><br>
            <input type="text" name="Tersedia" id="Tersedia" value="{{ old('Tersedia', $bolpen->Tersedia) }}">
        </p>
